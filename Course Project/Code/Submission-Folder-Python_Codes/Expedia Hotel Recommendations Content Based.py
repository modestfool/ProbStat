...
def preprocess_data():
        # In[ ]:
        # Data Cleaning - Read all the records in Train.csv and filter based on users' frequency
        train_set = pd.read_csv('../train.csv', parse_dates=['srch_ci', 'srch_co'])


        # In[ ]:
        # Print the train set information
        train_set.describe()
        ...
        print 'Total number of recods after filtering',len(filtered_train_set_10k)
        ...
        # In[ ]:
        #Merge both the PCA-destinations file and the Original dataframe
...
def get_cb_recommendations(train_set,test_set):
    ...
    print 'Accuracy with first value: ' ,avg_accuracy*1.0/len(filtered_with_recommendations)
    print 'Accuracy with all 5 recommendations: ' ,avg_accuracy_top*1.0/len(filtered_with_recommendations)
...
filtered_data_with_recos = get_cb_recommendations(filtered_data[:10000],filtered_data[:10000])

print 'Time to run the Content based recommendations ',datetime.fromtimestamp(int(time.time())) - curr_time

print filtered_data_with_recos[['hotel_cluster','cb_recommendations']].head(20)

curr_time  = datetime.fromtimestamp(int(time.time()))
print curr_time

k_fold(filtered_data[:10000])

print 'Time to run the Content based recommendations k-fold ',datetime.fromtimestamp(int(time.time())) - curr_time
